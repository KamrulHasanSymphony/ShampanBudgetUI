    ...
    public class ProductGroupRepo
    {
        ...
        public ResultVM ListAsDataTable(string[] conditionalFields, string[] conditionalValues, PeramModel vm = null)
        {
            try
            {
                ...
                #region Invoke API
                var data = httpRequestHelper.GetData("api/ProductGroup/ListAsDataTable", authModel);
                ResultVM result = JsonConvert.DeserializeObject<ResultVM>(data);
                #endregion
        ...
        public ResultVM Dropdown()
        {
            try
            {
                ...
                #region Invoke API
                var data = httpRequestHelper.GetData("api/ProductGroup/Dropdown", authModel);
                ResultVM result = JsonConvert.DeserializeObject<ResultVM>(data);
                #endregion
        ...
        public Stream ReportPreview(CommonVM model)
        {
            try
            {
                HttpRequestHelper httpRequestHelper = new HttpRequestHelper();
                AuthModel authModel = httpRequestHelper.GetAuthentication(new CredentialModel { UserName = "erp", Password = "123456" });
                var result = httpRequestHelper.PostDataReport("api/ProductGroup/ReportPreview", authModel, JsonConvert.SerializeObject(model));
