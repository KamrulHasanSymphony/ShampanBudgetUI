...
{
    public class BudgetOtherDepartmentController : Controller
    {
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    ...
                    if (model.Operation.ToLower() == "add")
                    {
                        ...
                        {
                            model = JsonConvert.DeserializeObject<BudgetOtherDepartmentHeaderVM>(resultVM.DataVM.ToString());
        ...
        public ActionResult BudgetTransferInsert(BudgetOtherDepartmentHeaderVM model)
        {
            ResultModel<BudgetOtherDepartmentHeaderVM> result = new ResultModel<BudgetOtherDepartmentHeaderVM>();
            ResultVM resultVM = new ResultVM { Status = MessageModel.Fail, Message = "Error", ExMessage = null, Id = "0", DataVM = null };
            _repo = new BudgetOtherDepartmentRepo();
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    ...
                    model.ToFiscalYearId = model.FiscalYearId;
